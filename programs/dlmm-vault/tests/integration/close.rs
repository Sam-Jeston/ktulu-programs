...
use crate::helpers::withdraw_ix::withdraw_all_vault_ix;
...
#[test]
fn test_withdraw_all_and_close() {
    let user = SKeypair::new();
    let user_clone = Keypair::from_bytes(&user.to_bytes()).unwrap();
    ...
    let token_x_initial_balance = 1000000;
    let token_y_initial_balance = 1000000;
    ...
    // Add a deposit instruction to simulate balance changes in the tx
    ...
    let withdraw_ix = withdraw_all_vault_ix(
        &user_clone,
        &vault_pda,
        &user_ata_x,
        &vault_ata_x,
    ...
    let tx = prepare_tx(
        ...
        &user.pubkey(),
        &[&user],
        &[deposit_ix, withdraw_ix, close_ix],
    );
