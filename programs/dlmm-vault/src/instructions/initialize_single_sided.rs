...
use crate::{
    events::initialize_single_sided::InitializeSingleSidedEvent, FeeCompoundingStrategy,
    Initialize, SingleSidedStrategy, VaultErrorCode, VolatilityStrategy,
};

// Single sided strategy to bid
pub fn handle_initialize_single_sided<'a, 'b, 'c, 'info>(
    ...
    if amount_x > 0 && amount_y > 0 {
        return Err(error!(VaultErrorCode::InvalidSingleSidedConfiguration));
    }

    // Bid with stop means we must only be providing token y
    if single_sided_strategy == SingleSidedStrategy::BidWithStoplossOnDownside && amount_x > 0 {
        return Err(error!(VaultErrorCode::InvalidSingleSidedConfiguration));
    }
    ...
    emit!(InitializeSingleSidedEvent {
        ...
        amount_x: amount_x,
        amount_y: amount_y,
        single_sided_strategy: single_sided_strategy,
    });
